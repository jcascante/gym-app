...
class Program(BaseModel):
    ...
    # TrainGen Engine link (set when program was generated from engine)
    engine_program_id = Column(
        String(100),
        nullable=True,
        doc="Engine program-definition ID used to generate this program",
    )

    engine_program_version = Column(
        String(50),
        nullable=True,
        doc="Version of the engine definition at generation time",
    )
...
class ProgramDay(BaseModel):
    ...
    # Computed when program is assigned (start_date + suggested_day_of_week offset)
    scheduled_date = Column(
        Date,
        nullable=True,
        doc="Absolute date this day is scheduled for (set at assignment time)",
    )
